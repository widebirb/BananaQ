...
async def get_readme(
    ...
    """Fetch README.md from the given ref.

    Returns (content_as_text, sha) where sha is needed for the subsequent PUT.
    Raises httpx.HTTPStatusError on failure.
    """
    import base64

    url = f"{GITHUB_API}/repos/{owner}/{repo}/contents/README.md"
    ...
    content_b64: str = data["content"]  # base64-encoded, may have newlines
    sha: str = data["sha"]
    content = base64.b64decode(content_b64).decode("utf-8")
    logger.info("Fetched README.md from %s/%s@%s (sha=%s).", owner, repo, ref, sha[:7])
    return content, sha
...
async def update_readme(
    ...
    """Commit updated README.md content directly to the given branch.

    content: full new file content as a plain string.
    sha: the blob SHA from the previous get_readme() call (required by GitHub API).
    """
    import base64
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        response = await client.put(
            url, headers=_headers(settings.github_token), json=payload
        )
        if response.status_code not in (200, 201):
            logger.error(
                "Failed to update README.md: %s %s", response.status_code, response.text
            )
            response.raise_for_status()

    logger.info("Committed updated README.md to %s/%s@%s.", owner, repo, branch)
